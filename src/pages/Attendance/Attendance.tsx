...
import './Attendance.css';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
// ... import your FontAwesome icons
import { useAttendance } from '../../hooks/useAttendance';
import { AttendanceRecord, MealRecord, HostelRecord } from '../../types/attendance.types';
...
const Attendance: React.FC = () => {
  ...
  // Custom hook handles all data fetching and transformation!
  const { summaryData, records, isLoading, error } = useAttendance(selectedMonth);

  // --- Helper Functions ---
  const getMealStatus = (mealRecord: MealRecord | HostelRecord) => {
    if (!mealRecord) return { present: 0, total: 0 };
    const values = Object.values(mealRecord);
    const presentCount = values.filter(val => val === true).length;
    return { present: presentCount, total: values.length };
  };
  ...
  // --- Render Handlers ---
  if (isLoading) return <div className="loading-state">Loading attendance data...</div>;
  if (error || !summaryData) return <div className="error-state">{error || "No data available."}</div>;

  // The rest of your rendering functions remain largely identical, 
  // safely consuming `summaryData` and `filteredRecords`
  ...
  const renderRulesTab = () => (
    // ... Returns your exact static rules component HTML as before
    // (Omitted here for brevity, keep the exact implementation from your original JSX)
    <div className="rules-tab">
       <div className="rules-categories">
        {attendanceRules.map((category, index) => (
          <div key={index} className="rules-category">
            <h4><FontAwesomeIcon icon={faExclamationTriangle} /> {category.category}</h4>
            <div className="rules-list">
              {category.rules.map((rule, ruleIndex) => (
                <div key={ruleIndex} className="rule-item">
                  <FontAwesomeIcon icon={faCheckCircle} className="rule-icon" />
